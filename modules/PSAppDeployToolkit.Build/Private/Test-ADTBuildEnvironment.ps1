...
function Test-ADTBuildEnvironment
{
    ...
    try
    {
        ...
        if (Get-ChildItem -LiteralPath $env:PSModulePath.Split([System.IO.Path]::PathSeparator, [System.StringSplitOptions]::RemoveEmptyEntries).Where({ ![System.String]::IsNullOrWhiteSpace($_) }).Trim() -Filter $Script:ModuleConstants.ModuleName -ErrorAction Ignore)
        {
