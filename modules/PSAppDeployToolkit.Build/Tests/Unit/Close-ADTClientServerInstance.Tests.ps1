...
Describe 'Close-ADTClientServerInstance' {
    ...
    Context 'With no client running' {
        ...
        It 'Reports that there is nothing to close' {
            ...
            InModuleScope -ModuleName PSAppDeployToolkit {
                { Close-ADTClientServerInstance } | Should -Throw -ErrorId 'ClientServerInstanceNotFoundError,Close-ADTClientServerInstance'
            }
        }

        It 'Leaves the module holding no client either way' {
            InModuleScope -ModuleName PSAppDeployToolkit {
                try { Close-ADTClientServerInstance } catch { $null = $_ }
