...
function Initialize-ADTFunction
{
    ...
    if ($InformationPreference -notmatch '^(SilentlyContinue|Ignore)$')
    {
        ...
        if ($Cmdlet.MyInvocation.BoundParameters.Count)
        {
            ...
            $CmdletBoundParameters = $Cmdlet.MyInvocation.BoundParameters | Format-Table -Property @{ Label = 'Parameter'; Expression = { "[-$($_.Key)]" } }, @{ Label = 'Value'; Expression = { if ($secureArgumentList -and $_.Key.EndsWith('ArgumentList') -and !$_.Key.Equals('SecureArgumentList')) { '<Value Hidden>' } else { $_.Value } }; Alignment = 'Left' }, @{ Label = 'Type'; Expression = { if ($_.Value) { $_.Value.GetType().Name } }; Alignment = 'Left' } -AutoSize -Wrap | Out-String
