...
function Export-ADTEnvironmentTableToSessionState
{
    ...
    process
    {
        try
        {
            try
            {
                $null = $ExecutionContext.InvokeCommand.InvokeScript($SessionState, { $args[1] | . { process { & $args[0] -Name $_.Name -Value $_.Value -Option ReadOnly -Force } } $args[0] }.Ast.GetScriptBlock(), (Get-ADTCommand -Name New-Variable), $adtEnv.PSObject.Properties)
            }
            catch
            {
