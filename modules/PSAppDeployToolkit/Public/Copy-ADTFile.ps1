...
function Copy-ADTFile
{
    ...
    .LINK
        https://github.com/PSAppDeployToolkit/PSAppDeployToolkit/blob/main/modules/PSAppDeployToolkit/Public/Copy-ADTFile.ps1
    #>
            ...
            foreach ($srcPath in $PSBoundParameters.($PSCmdlet.ParameterSetName))
            {
                try
                {
                    try
                    {
                        ...
                        {
                            Write-ADTLogEntry -Message "The following warnings were detected while copying file(s) in path [$srcPath] to destination [$Destination].`n`n$([System.String]::Join([System.Environment]::NewLine, $FileCopyError.Exception.Message))" -Severity Warning
                        }
                        else
                        {
