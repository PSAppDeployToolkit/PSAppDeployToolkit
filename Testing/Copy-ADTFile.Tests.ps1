BeforeAll {
	Import-Module "$PSScriptRoot\..\src\PSAppDeployToolkit\PSAppDeployToolkit.psd1"
}
...
Describe 'Copy-ADTFile'-ForEach @(
	...
	It 'Quits copying files when encountering an error ($FileCopyMode = $<FileCopyMode>)' {
		Copy-ADTFile -Path @("$SourcePath\test.txt", "$SourcePath\Subfolder99\test99.txt", "$SourcePath\Subfolder2\test2.txt") -Destination $DestinationPath -FileCopyMode $FileCopyMode
	...
	It 'Continues copying files when encountering an error ($FileCopyMode = $<FileCopyMode>)' {
		Copy-ADTFile -Path @("$SourcePath\test.txt", "$SourcePath\Subfolder99\test99.txt", "$SourcePath\Subfolder2\test2.txt") -Destination $DestinationPath -FileCopyMode $FileCopyMode -ContinueFileCopyOnError
