...
namespace PSADT.ClientServer.Server.Tests.TestHelpers
{
    ...
    /// </summary>
    /// <remarks>
    /// Read from the type's fields rather than through <c>Enum.GetValues</c>, which reports the set of
    /// values and so cannot answer whether two names were given the same one - and rather than through the
    /// generic <c>Enum.GetValues&lt;T&gt;</c>, which is not available on every target framework this
    /// project builds for.
    /// </remarks>
