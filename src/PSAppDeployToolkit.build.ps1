...
Add-BuildTask DotNetBuild {
    ...
    if (!($msbuildPath = [System.IO.FileInfo[]](& "$([System.Environment]::GetFolderPath('ProgramFilesX86'))\Microsoft Visual Studio\Installer\vswhere.exe" -requires Microsoft.Component.MSBuild -find MSBuild\Current\Bin\MSBuild.exe) | Sort-Object -Property { [System.Version]$_.VersionInfo.FileVersion } -Descending | Select-Object -First 1 -ExpandProperty FullName))
    {
    ...
    foreach ($buildItem in $Script:buildItems)
    {
        ...
        foreach ($buildType in ($buildConfigs | Select-Object -Unique))
        {
            ...
            dotnet.exe test $solutionPath -c $buildType -p:Platform="Any CPU" --no-build
