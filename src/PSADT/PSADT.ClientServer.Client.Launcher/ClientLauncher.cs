...
namespace PSADT.ClientServer
{
    ...
    internal static class ClientLauncher
    {
        ...
        private static int Main(string[] argv)
        {
            ...
            try
            {
                return ProcessManager.LaunchAsync(new(EnvironmentInfo.ClientServerClientPath, argv.Length > 0 ? argv : null, argv.Any(static arg => arg.EndsWith(BlockExecution.RegistryKeyName, StringComparison.OrdinalIgnoreCase)) ? Environment.CurrentDirectory : Environment.SystemDirectory, denyUserTermination: true, useShellExecute: true, windowStyle: ProcessWindowStyle.Hidden))!.Task.GetAwaiter().GetResult().ExitCode;
            }
