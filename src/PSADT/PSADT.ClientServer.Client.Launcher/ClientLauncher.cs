...
namespace PSADT.ClientServer
{
    ...
    internal static class ClientLauncher
    {
        ...
        private static int Main(string[] argv)
        {
            ...
            try
            {
                return ProcessManager.LaunchAsync(new(typeof(ClientLauncher).Assembly.Location.Replace(".Launcher.exe", ".exe", StringComparison.OrdinalIgnoreCase), argv.Length > 0 ? argv : null, Environment.SystemDirectory, denyUserTermination: true, createNoWindow: true))!.Task.GetAwaiter().GetResult().ExitCode;
            }
