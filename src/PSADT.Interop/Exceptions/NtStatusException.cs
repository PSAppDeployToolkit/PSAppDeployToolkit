...
namespace PSADT.Interop.Exceptions
{
    ...
    internal sealed class NtStatusException : ExternalException
    {
        ...
        private static string GetMessageForNtStatus(NTSTATUS ntStatus)
        {
            ...
            try
            {
                return $"{FormatMessageRemovalRegex.Replace(buffer[..(int)NativeMethods.FormatMessage(FormatMessageFlags, NtDllHandle, unchecked((uint)ntStatus.Value), buffer)].ToString(), string.Empty).AsSpan().Trim().TrimEnd('.')}. {messageSuffix}";
            }
