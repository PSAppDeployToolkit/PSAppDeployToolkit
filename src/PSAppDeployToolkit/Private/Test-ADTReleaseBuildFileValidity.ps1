...
function Private:Test-ADTReleaseBuildFileValidity
{
    ...
    if ($Script:Module.Compiled -and $Script:Module.Signed -and ($badFiles = Get-ChildItem @PSBoundParameters -Filter *.ps*1 -Recurse | Get-AuthenticodeSignature | & { process { if (!$_.get_Status().Equals([System.Management.Automation.SignatureStatus]::Valid)) { return $_ } } }))
    {
        return $badFiles
    }
}
