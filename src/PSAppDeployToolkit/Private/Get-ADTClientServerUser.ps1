...
function Private:Get-ADTClientServerUser
{
    ...
    {
        [PSADT.Foundation.RunAsActiveUser]::GetAsync().GetAwaiter().GetResult()
    }
    ...
    if ($runAsActiveUser)
    {
        ...
        if (!$runAsActiveUser.SID.Equals([PSADT.AccountManagement.AccountUtilities]::CallerSid) -and ![PSADT.AccountManagement.AccountUtilities]::CallerIsLocalSystem -and [System.Environment]::UserInteractive -and ($null -ne ($missingPermissions = [PSADT.Interop.SE_PRIVILEGE]::SeDebugPrivilege, [PSADT.Interop.SE_PRIVILEGE]::SeIncreaseQuotaPrivilege, [PSADT.Interop.SE_PRIVILEGE]::SeAssignPrimaryTokenPrivilege | & { begin { $callerPrivileges = [PSADT.AccountManagement.AccountUtilities]::CallerPrivileges } process { if (!$callerPrivileges.Contains($_)) { return $_ } } })))
        {
