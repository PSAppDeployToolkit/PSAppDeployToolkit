            ...
            try
            {
                ...
                if ($PurgeActiveSetupKey)
                {
                    ...
                    if ($runAsActiveUser)
                    {
                        ...
                        & $Script:CommandTable.'Invoke-ADTAllUsersRegistryChange' -UserProfiles (& $Script:CommandTable.'Get-ADTUserProfiles' -ExcludeDefaultUser | & { process { if ($_.SID -eq $runAsActiveUser.SID) { return $_ } } } | & $Script:CommandTable.'Select-Object' -First 1) -RegistrySettings {
                ...
                if (![System.Diagnostics.Process]::GetCurrentProcess().SessionId)
                {
                    ...
                    if ($CUArguments)
                    {
                        & $Script:CommandTable.'Start-ADTProcessAsUser' -FilePath $CUStubExePath -ArgumentList $CUArguments -Wait -PrimaryActiveUserSession -HideWindow
                    }
                    else
                    {
                        & $Script:CommandTable.'Start-ADTProcessAsUser' -FilePath $CUStubExePath -Wait -PrimaryActiveUserSession -HideWindow
                    }
