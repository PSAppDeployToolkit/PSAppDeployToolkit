        ...
        try
        {
            try
            {
                ...
                if (($pathIsProductCode = $Path -match (& $Script:CommandTable.'Get-ADTEnvironment').MSIProductCodeRegExPattern))
                {
                    ...
                    $productCodeNameVersion = & $Script:CommandTable.'Get-ADTInstalledApplication' -FilterScript { $_.ProductCode -eq $Path } -IncludeUpdatesAndHotfixes:$IncludeUpdatesAndHotfixes | & $Script:CommandTable.'Select-Object' -Property Publisher, DisplayName, DisplayVersion -First 1 -ErrorAction Ignore
                ...
                elseif ([System.IO.Path]::GetExtension($msiFile) -eq '.msi')
                {
                    try
                    {
                        ...
                        & $Script:CommandTable.'Get-ADTMsiTableProperty' @GetMsiTablePropertySplat | & $Script:CommandTable.'Select-Object' -ExpandProperty ProductCode
                    }
                    catch
                    {
                ...
                {
                    !!(& $Script:CommandTable.'Get-ADTInstalledApplication' -FilterScript { $_.ProductCode -eq $MSIProductCode } -IncludeUpdatesAndHotfixes:$IncludeUpdatesAndHotfixes)
                }
                else
                {
                    $Action -ne 'Install'
