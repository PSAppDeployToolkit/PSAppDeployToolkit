...
function Test-ADTMutexAvailability
{
    ...
    process
    {
        ...
        finally
        {
            ...
            {
                Write-ADTLogEntry -Message "Mutex [$MutexName] is not available for an exclusive lock because the following MSI installation is in progress [$($msiInProgress.FilePath) $([System.String]::Join(' ', $msiInProgress.ArgumentList))]." -Severity Warning
            }
            else
            {
