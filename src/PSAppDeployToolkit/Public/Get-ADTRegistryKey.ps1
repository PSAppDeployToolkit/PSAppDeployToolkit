...
function Get-ADTRegistryKey
{
    ...
    .DESCRIPTION
        ...
        To test for the existence of a registry key path, use built-in `Test-Path` cmdlet. To test for the existence of a registry key value, use the `Test-ADTRegistryValue` function.
                ...
                $(try { Get-Item @pathParam } catch { Get-Item @pathParam }) | & {
                    process
                    {
                        ...
                        if (![System.String]::IsNullOrWhiteSpace($Name))
                        {
                            ...
                            {
                                return $_.GetValue($(if ($Name -ne '(Default)') { $Name } else { [System.Management.Automation.Language.NullString]::Value }), $null, [Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames)
                            }
                            elseif ($Name -like '(Default)')
                            {
                                return $_.GetValue([System.Management.Automation.Language.NullString]::Value)
                            }
                            else
                            {
