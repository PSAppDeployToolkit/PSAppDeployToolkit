...
function Remove-ADTHashtableNullOrEmptyValues
{
    ...
    .DESCRIPTION
        The `Remove-ADTHashtableNullOrEmptyValues` function returns a new hashtable that contains entries from the source where the value is not null, empty, or consists only of whitespace. When the `-Recurse` switch is specified, the function will also traverse any nested hashtables and apply the same filtering to them.
