        ...
        try
        {
            try
            {
                ...
                if ($adtSession)
                {
                    ...
                    Start-Process -FilePath (Get-ADTPowerShellProcessPath) -ArgumentList "-ExecutionPolicy Bypass -NonInteractive -NoProfile -NoLogo -WindowStyle Hidden -Command & (Import-Module -Name '$($Script:PSScriptRoot)\$($MyInvocation.MyCommand.Module.Name).psd1' -PassThru) { & `$CommandTable.'Initialize-ADTModule' -ScriptDirectory '$((Get-ADTModuleData).ScriptDirectory)'; `$null = & `$DialogDispatcher.$($adtConfig.UI.DialogStyle).'$($MyInvocation.MyCommand.Name)' $(($PSBoundParameters | Resolve-ADTBoundParameters -Exclude SilentRestart, SilentCountdownSeconds).Replace('"', '\"')) }" -WindowStyle Hidden -ErrorAction Ignore
                    return
                }
