    ...
    process
    {
        try
        {
            try
            {
                ...
                if ($NoWait)
                {
                    Start-Process -FilePath (Get-ADTPowerShellProcessPath) -ArgumentList "$(if (!(Test-ADTModuleIsReleaseBuild)) { "-ExecutionPolicy Bypass " })-NonInteractive -NoProfile -NoLogo -WindowStyle Hidden -Command & (Import-Module -FullyQualifiedName @{ ModuleName = '$($Script:PSScriptRoot)\$($MyInvocation.MyCommand.Module.Name).psd1'; Guid = '$($MyInvocation.MyCommand.Module.Guid)'; ModuleVersion = '$($MyInvocation.MyCommand.Module.Version)' } -PassThru) { & `$CommandTable.'Initialize-ADTModule' -ScriptDirectory '$((Get-ADTModuleData).Directories.Script)'; `$null = & `$CommandTable.'$($MyInvocation.MyCommand.Name)$($adtConfig.UI.DialogStyle)' $(($PSBoundParameters | Resolve-ADTBoundParameters -Exclude NoWait).Replace('"', '\"')) -ADTConfig (& `$CommandTable.'Get-ADTConfig') }" -WindowStyle Hidden -ErrorAction Ignore
                    return
                }
                ...
                & $Script:CommandTable."$($MyInvocation.MyCommand.Name)$($adtConfig.UI.DialogStyle)" @PSBoundParameters -ADTConfig $adtConfig
            }
            catch
            {
