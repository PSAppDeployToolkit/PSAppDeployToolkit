    ...
    process
    {
        ...
        try
        {
            try
            {
                ...
                $blockExecArgs.Add('Options', [PSADT.Serialization.JsonSerialization]::SerializeToString([PSADT.UserInterface.DialogOptions.CustomDialogOptions]$dialogOptions))
                ...
                Set-ADTRegistryKey -Key $blockExecRegPath -Name BlockExecutionCommand -Value ([PSADT.Serialization.JsonSerialization]::SerializeToString($blockExecArgs)) -InformationAction SilentlyContinue
