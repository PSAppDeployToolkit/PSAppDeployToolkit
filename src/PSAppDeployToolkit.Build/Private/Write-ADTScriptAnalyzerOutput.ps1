...
function Write-ADTScriptAnalyzerOutput
{
    ...
    {
        $output = ($DiagnosticRecord[$i] | Format-List -Property * | Out-String -Width ([System.Int32]::MaxValue)).Replace("`r", [System.Management.Automation.Language.NullString]::Value).Trim().Split(0x0A).Trim() -replace '^', "> "
